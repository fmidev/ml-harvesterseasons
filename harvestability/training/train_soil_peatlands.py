...
# Cleaning train data, eremoving nan, null, 0, 254 (no data), removing blank harvestibility classes
train_clean_data = train_data.loc[~train_data["harvestability"].isin(["",254,0,np.nan,1,2,3])]

# Selecting features to train
train_clean_data = train_clean_data[cols_own]

#converting  harvestability column to 0,1,2 classes
train_clean_data['harvestability'] = train_clean_data['harvestability'].map({4: 0, 5: 1,6:2})

# Cleaning test data, eremoving nan, null, 0, 254 (no data), removing blank harvestibility classes
test_clean_data = test_data.loc[~test_data["harvestability"].isin(["",254,0,np.nan,1,2,3])]

# selecting features to test
test_clean_data = test_clean_data[cols_own]

#converting  harvestability column to 0,1,2 classes
test_clean_data['harvestability'] = test_clean_data['harvestability'].map({4: 0, 5: 1,6:2})

train_clean_data = train_clean_data.groupby("harvestability").sample(n=31000, random_state=42)
...
xgb_cl = xgb.XGBClassifier(objective='multi:softmax', 
                            num_class=3,
                            learning_rate= 0.24851917112502248,
                            early_stopping_rounds=10,
                            max_depth=28,
                            n_estimators=273,
                            subsample=0.9939130835684477,
                            eval_metric="merror")
...
plt.savefig(plot_dir+'soil_peatland_importance.jpg', bbox_inches='tight')
...
plt.title('Harvestability Peatlands Confusion Matrix')
ax.set_ylabel('True Class')
ax.set_xlabel('Predicted Class')
plt.savefig(plot_dir+'Harvestability_Peatlands_Confusion_Matrix',bbox_inches='tight')
plt.close()
...
print("---Saving model---")
xgb_cl.save_model(f"{model_dir}xgbmodel_harvestability_peatland_{date_time}.json")
