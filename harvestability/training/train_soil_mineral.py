...
train_clean_data = train_data.loc[~train_data["harvestability"].isin(["",254,0,np.nan,4,5,6])]

# Selecting features to train
train_clean_data = train_clean_data[cols_own]

#converting  harvestability column to 0,1,2 classes
train_clean_data['harvestability'] = train_clean_data['harvestability'].map({1: 0, 2: 1,3: 2})

# Cleaning test data, eremoving nan, null, 0, 254 (no data), removing blank harvestibility classes
test_clean_data = test_data.loc[~test_data["harvestability"].isin(["",254,0,np.nan,4,5,6])]

# selecting features to test
test_clean_data = test_clean_data[cols_own]

#converting  harvestability column to 0,1,2 classes
test_clean_data['harvestability'] = test_clean_data['harvestability'].map({1: 0, 2: 1,3: 2})

train_clean_data = train_clean_data.groupby("harvestability").sample(n=40000, random_state=42)
...
xgb_cl = xgb.XGBClassifier(objective='multi:softmax', 
                            num_class=3,
                            learning_rate= 0.1841088000372496,
                            early_stopping_rounds=10,
                            max_depth=19,
                            n_estimators=183,
                            subsample=0.8129499667617793,
                            eval_metric="merror")
...
plt.savefig(plot_dir+'soil_mineral_importance.jpg', bbox_inches='tight')
...
plt.title('Harvestability Mineral Confusion Matrix')
ax.set_ylabel('True Class')
ax.set_xlabel('Predicted Class')
plt.savefig(plot_dir+'Harvestability_Mineral_Confusion_Matrix',bbox_inches='tight')
plt.close()
...
print("---Saving model---")
xgb_cl.save_model(f"{model_dir}xgbmodel_harvestability_mineral_{date_time}.json")
